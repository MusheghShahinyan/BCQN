...
void computeInjectiveStepSize_2d(const MatrixXd& F, const MatrixXd& x, const MatrixXd& p, double tol, double *output)
{
	int n_tri = F.rows();
...
void computeInjectiveStepSize_3d(const MatrixXd& F, const MatrixXd& x, const MatrixXd& p, double tol, double *output)
{
	int n_tri = F.rows();
	...
	int nrhs, const mxArray*prhs[])
{
    ...
    output_mex = plhs[0] = mxCreateDoubleMatrix(n_tri, 1, mxREAL);
    ...
    {
        //mexPrintf("correct\n");
		computeInjectiveStepSize_2d(F, x, p, *tol, output);
    }
	else if (dim == 3)
    {
        //mexPrintf("wrong\n");
		computeInjectiveStepSize_3d(F, x, p, *tol, output);
    }
	else
